...
class DDQNAgent:
    def __init__(self, action_dim, lr=1e-3, gamma=0.99, eps_decay=0.999,
                 buffer_size=300000, batch_size=64):
        self.device = device
...
def train(env, agent, episodes, potential, csv_seeds, max_steps=0, log_every=100):
    ...
    for ep in range(episodes):
        ...
        if ep % log_every == 0:
            ...
            print(f"Ep {ep:5d}: reward={ep_rew:.2f} avg100={avg_r:.2f} sr={np.mean(sr_buffer):.3f} eps={agent.epsilon:.3f} buf={len(agent.memory)} v_llm={log_hits} hits φavg={phi_avg:.2f}")
            log_hits = 0
            log_phi = 0.0
