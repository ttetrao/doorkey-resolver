...
DRY_RUN = False
HISTORY_PER_REQUEST = 3
NUM_REQUESTS = 300
RETRY_LIMIT = 3
RETRY_DELAY = 5
MODEL_ID = "qwen3.5-9b"  # LM Studio di solito ignora questo campo se è caricato un solo modello, ma è richiesto dal formato OpenAI
TEMPERATURE = 0.7

# Configurazione endpoint LM Studio
...
def call_with_retry(prompt, retries=RETRY_LIMIT, delay=RETRY_DELAY) -> str | None:
    ...
    for attempt in range(retries):
        try:
            # Usiamo timeout=None per attendere indefinitamente la generazione locale
            response = requests.post(
                LMSTUDIO_URL, headers=headers, json=payload, timeout=None
            )
            response.raise_for_status()  # Solleva un'eccezione per errori HTTP (es. 500, 404)
            ...
            print(
                f"  errore richiesta LM Studio (tentativo {attempt + 1}/{retries}): {e}"
            )
        except (KeyError, IndexError) as e:
            print(
                f"  errore formato risposta JSON (tentativo {attempt + 1}/{retries}): {e}"
            )
...
def build_base_prompt(base: str, doc: str, vfun_def: str) -> str:
    tmpl = open(os.path.join(base, "docs/en/Prompt-en.txt")).read()
    return tmpl.replace(
        "X<legend>",
        f"<documentation>\n{doc}\n</documentation>\n<definition>\n{vfun_def}\n</definition>\n<legend>",
    )
...
def run():
    print(f"Utilizzo server locale LM Studio: {LMSTUDIO_URL}")
