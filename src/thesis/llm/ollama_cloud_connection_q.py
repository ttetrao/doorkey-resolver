...
def call_with_retry(
    ...
    for attempt in range(retries):
        try:
            # Usiamo l'API di chat della libreria ufficiale di Ollama.
...
def build_base_prompt(base: str, doc: str, vfun_def: str) -> str:
    # Carichiamo il prompt per la Q-function
...
def run():
    # Inizializza il client Ollama usando l'endpoint cloud e la chiave API
    client = Client(
    ...
    # Leggiamo il metadata.csv e dividiamo gli stati (entry_type=0) dalle q_values reali (entry_type=1)
    ...
    if DRY_RUN:
        ...
        for e in states:
            print(f"  {e.id}: seed={e.seed}, event={e.event}")
    ...
    for batch_idx, batch in enumerate(batches, start=1):
        ...
        parsed_entries = parse_response(response, batch, real_q_lookup)
