...
class QLearningAgent:
    def __init__(self, n_actions, alpha=0.3, gamma=0.99, epsilon=1.0,
                 epsilon_min=0.05, epsilon_decay=0.997, warm=()):
        ...
        for s, v in warm.items():
            self.q[s] = np.full(n_actions, v, dtype=np.float32)
...
def train(env, agent, episodes, potential, csv_seeds, max_steps=0, log_every=100):
    ...
    for ep in range(episodes):
        ...
        if ep % log_every == 0:
            ...
            print(f"Ep {ep:5d}: reward={ep_rew:.2f} avg100={avg_r:.2f} sr={np.mean(sr_buffer):.3f} eps={agent.epsilon:.3f} states={len(agent.q)} v_llm={log_hits} hits φavg={phi_avg:.2f}")
            log_hits = 0
            log_phi = 0.0
...
def main():
    parser = argparse.ArgumentParser(description="Q-Learning tabulare con warm start della Q-table (Q(s,a)=V_known per s noti)")
    parser.add_argument("--episodes", type=int, default=3000)
    parser.add_argument("--alpha", type=float, default=0.3)
    parser.add_argument("--gamma", type=float, default=0.99)
    parser.add_argument("--eps_decay", type=float, default=0.997)
    ...
    parser.add_argument("--compare", action="store_true", help="train vanilla (tabella a 0) e warm start, test e confronta")
    ...
    if args.compare:
        ...
        random.seed(args.seed); np.random.seed(args.seed)
        agent_w = make_agent(known)
        pot_w = build_potential(known) if args.shaping else build_potential({})
        hist_w = train(env, agent_w, args.episodes, pot_w, csv_seeds, args.max_steps, args.log_every)
        eval_w = evaluate(env, agent_w, len(eval_seeds), eval_seeds)
        print(f"Warm-start eval SR: {eval_w:.3f}")

        plot_compare(hist_v, hist_w, eval_v, eval_w, "qtable_warmstart_compare.png")
        env.close()
        return

    agent = make_agent(known)
    pot = build_potential(known) if args.shaping else build_potential({})
    print(f"Training Q-Learning warm-start: {args.episodes} episodes, alpha={args.alpha}, shaping={args.shaping}")
    train(env, agent, args.episodes, pot, csv_seeds, args.max_steps, args.log_every)
    env.close()
