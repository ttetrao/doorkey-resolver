...
class QLearningAgent:
    def __init__(self, n_actions, alpha=0.15, gamma=0.99, epsilon=1.0,
...
def main():
    parser = argparse.ArgumentParser(description="Q-Learning tabulare MiniGrid-DoorKey con shaping v_llm")
    ...
    if args.compare:
        ...
        plot_compare(hist_v, hist_a, eval_v, eval_a, "qtable_compare.png")
        env.close()
        return

    agent = make_agent()
    print(f"Training Q-Learning: {args.episodes} episodes, alpha={args.alpha}, gamma={args.gamma}")
    train(env, agent, args.episodes, build_potential(known), csv_seeds, args.max_steps, args.log_every)
    env.close()
