...
def call_with_retry(
    ...
    for attempt in range(retries):
        try:
            # Usiamo l'API di chat della libreria ufficiale.
            # stream=False ci restituisce l'intero messaggio in un solo dizionario.
            ...
            print(f"  risposta grezza: {content[:100]}...")  # Anteprima per log
...
def run():
    ...
    # Sostituisce il carattere ":" per evitare problemi nel nome del file CSV
    safe_model_name = MODEL_ID.replace(":", "_")
    out_path = os.path.join(base, "files", f"output_ollama_{safe_model_name}.csv")
